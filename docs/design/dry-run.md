...
- Requiring the label is acceptable; Users adjust their testing Promise.
...
1. **Does Users' compound emit components declaratively?** If a pipeline creates
...
3. **The pipeline is identical in both modes**; only Kratix's interpretation changes.
   ...
   `DryRun` CRs directly.
4. **Users need a one-line pipeline change** to label the component requests they
   already emit.
